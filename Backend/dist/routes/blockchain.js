import { ipRegistryService, courseDirectoryService, certificateService, reputationService, royaltyService, blockchainUtils, } from '../services/blockchainService.js';
// ============ Routes ============
export const blockchainRoutes = async (app) => {
    // ===== Health Check =====
    /**
     ...
     */
    app.post('/api/blockchain/courses/register', async (request, reply) => {
        try {
            ...
            const result = await ipRegistryService.registerCourse(metadataHash, tags || [], royaltyBps || 500);
     ...
     */
    app.get('/api/blockchain/courses/:id', async (request, reply) => {
        try {
     ...
     */
    app.get('/api/blockchain/courses/creator/:wallet', async (request, reply) => {
        try {
     ...
     */
    app.get('/api/blockchain/courses/total', async (request, reply) => {
        try {
     ...
     * Enroll in a course
     */
    app.post('/api/blockchain/courses/:id/enroll', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
            if (isNaN(courseId)) {
                return reply.status(400).send({
     ...
     */
    app.post('/api/blockchain/courses/:id/complete', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
            if (isNaN(courseId)) {
                return reply.status(400).send({
     ...
     */
    app.post('/api/blockchain/courses/:id/rate', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
     ...
     */
    app.get('/api/blockchain/courses/:id/enrollment/:wallet', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
            const { wallet } = request.params;
            if (isNaN(courseId)) {
     ...
     */
    app.post('/api/blockchain/certificates/mint', async (request, reply) => {
        try {
     ...
     */
    app.get('/api/blockchain/certificates/:tokenId', async (request, reply) => {
        try {
     ...
     */
    app.get('/api/blockchain/certificates/learner/:wallet', async (request, reply) => {
        try {
            const { wallet } = request.params;
            if (!/^0x[a-fA-F0-9]{40}$/.test(wallet)) {
                return reply.status(400).send({
            ...
            const certificates = await Promise.all(tokenIds.map(async (tokenId) => {
     ...
     */
    app.get('/api/blockchain/certificates/total', async (request, reply) => {
        try {
     ...
     */
    app.get('/api/blockchain/reputation/:wallet', async (request, reply) => {
        try {
            const { wallet } = request.params;
            if (!/^0x[a-fA-F0-9]{40}$/.test(wallet)) {
                return reply.status(400).send({
            ...
            // Fetch badge details
            const badges = await Promise.all(badgeIds.map(async (badgeId) => {
     ...
     */
    app.post('/api/blockchain/reputation/:wallet/award-completion', async (request, reply) => {
        try {
            const { wallet } = request.params;
     ...
     * Purchase a course
     */
    app.post('/api/blockchain/courses/:id/purchase', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
     ...
     */
    app.post('/api/blockchain/courses/:id/price', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
            const { priceInEth } = request.body;
            if (isNaN(courseId)) {
     ...
     * Get course price
     */
    app.get('/api/blockchain/courses/:id/price', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
            if (isNaN(courseId)) {
                return reply.status(400).send({
     ...
     */
    app.get('/api/blockchain/earnings/:wallet', async (request, reply) => {
        try {
            const { wallet } = request.params;
            if (!/^0x[a-fA-F0-9]{40}$/.test(wallet)) {
                return reply.status(400).send({
     ...
     */
    app.post('/api/blockchain/earnings/withdraw', async (request, reply) => {
        try {
     ...
     */
    app.get('/api/blockchain/courses/:id/purchase-status/:wallet', async (request, reply) => {
        try {
            const courseId = parseInt(request.params.id);
            const { wallet } = request.params;
            if (isNaN(courseId)) {
