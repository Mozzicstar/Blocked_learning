...
export const courseRoutes = async (app) => {
     ...
     */
    app.post('/api/courses/upload', async (request, reply) => {
        try {
            // TODO: Add auth middleware here
            // For now, require wallet in body
            const { title, description, tags, fileCid, fileName } = request.body;
            const creatorWallet = request.headers['x-wallet'] || '0x0000000000000000000000000000000000000000';
     ...
     */
    app.post('/api/courses/publish', async (request, reply) => {
        try {
            ...
            if (!ipTokenId && blockchainUtils.isConfigured()) {
                ...
                try {
                    const result = await ipRegistryService.registerCourse(onchainMetadataHash, tags, royaltyBps);
            ...
            const publishedCourse = await courseService.publishCourse(courseId, finalIpTokenId, finalMetadataHash, finalTxHash);
            return reply.status(200).send({
                statusCode: 200,
     ...
     */
    app.get('/api/courses/onchain', async (request, reply) => {
        try {
            ...
            if (blockchainUtils.isConfigured()) {
                enrichedCourses = await Promise.all(dbCourses.map(async (course) => {
                    if (course.ip_token_id) {
                        try {
                            const onchainCourse = await ipRegistryService.getCourse(parseInt(course.ip_token_id));
                            return {
                                ...course,
                                onchain: {
     ...
     */
    app.get('/api/courses/creator/:wallet', async (request, reply) => {
        try {
