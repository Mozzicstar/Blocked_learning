...
export const authService = {
     ...
     */
    storeNonceForWallet: (nonce, wallet) => {
        const data = nonceStore.get(nonce);
        if (data) {
            data.wallet = wallet;
        }
    ...
    getOrCreateUser: async (wallet) => {
        try {
            ...
            const result = await db.query('SELECT * FROM users WHERE wallet = $1', [wallet]);
            if (result.rows.length > 0) {
                return result.rows[0];
            }
            // Create new user
            const insertResult = await db.query('INSERT INTO users (wallet) VALUES ($1) RETURNING *', [wallet]);
